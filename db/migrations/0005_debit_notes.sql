-- Migration: Add DebitNote and DebitNoteItem tables
-- This migration adds support for debit notes (used when replacing products but customer needs to pay more)
-- 
-- IMPORTANT: Run this migration AFTER supabase-schema.sql and supabase-migration-invoice-sequence.sql
-- This migration only creates DebitNote and DebitNoteItem tables, no types or enums.

-- Add debitNotes relation column to Invoice table if it doesn't exist
-- (This is handled by the foreign key relationship, but we ensure the column exists)
...
-- CreateIndex: Unique constraint on debitNoteNumber
CREATE UNIQUE INDEX IF NOT EXISTS "DebitNote_debitNoteNumber_key" ON "DebitNote"("debitNoteNumber");

-- CreateIndex: Unique constraint on debitNoteNumber and companyId
