...
export default function ClientsImportPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsParsing(true);
    try {
      const rows = await parseClientsCsv(selected);
      setParsedRows(rows);
    } catch (err) {
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent<HTMLDivElement>) => {
      ...
      setIsParsing(true);
      try {
        const rows = await parseClientsCsv(dropped);
        setParsedRows(rows);
      } catch (err) {
        toast.error((err as Error).message || "Грешка при парсване на CSV.");
      } finally {
  ...
  const handleDownloadTemplate = () => {
    const csv = generateClientsCsvTemplate();
    downloadCsv(csv, "clients_import_template.csv");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="page-title">Импорт на клиенти</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Качете CSV файл с данни за клиенти. Изтеглете шаблон за правилен формат.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div
              ...
              >
                {importLock.isPending ? (
                  ...
                  <>
                    ...
                    Импортирай {validRows.length} клиента
                  </>
                )}
              </Button>
            </div>
          ...
          <CardContent className="text-sm space-y-3">
            <div>
              ...
              <ul className="list-disc pl-5 text-muted-foreground space-y-0.5">
                <li><code>name</code> — име на клиента</li>
              </ul>
            </div>
            <div>
              <p className="font-medium mb-1">Препоръчително:</p>
              <ul className="list-disc pl-5 text-muted-foreground space-y-0.5">
                <li><code>bulstatNumber</code> — ЕИК/БУЛСТАТ</li>
                <li><code>email</code>, <code>phone</code></li>
                <li><code>vatRegistered</code> — true/false</li>
                <li><code>vatRegistrationNumber</code></li>
                <li><code>mol</code> — МОЛ</li>
                <li><code>address</code>, <code>city</code>, <code>zipCode</code>, <code>country</code></li>
              </ul>
            </div>
          </CardContent>
        </Card>
