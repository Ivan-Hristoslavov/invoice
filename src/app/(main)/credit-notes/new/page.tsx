    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = data.details 
          ? `${data.error || 'Грешка при създаване на кредитно известие'}: ${JSON.stringify(data.details)}`
          : data.error || 'Грешка при създаване на кредитно известие';
        throw new Error(errorMessage);
      }

      toast.success('Кредитното известие е създадено успешно', {
        description: `Номер: ${data.creditNote.creditNoteNumber}`,
      });

      router.push(`/credit-notes/${data.creditNote.id}`);
    } catch (error: any) {
      console.error('Error creating credit note:', error);
      toast.error(error.message || 'Грешка при създаване на кредитно известие');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold mt-4">Ново кредитно известие</h1>
          <p className="text-muted-foreground mt-1">
            Създайте кредитно известие за възстановяване на пари при връщане на продукт
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Например: Връщане на продукт и възстановяване на пари"
                    rows={3}
                  />
                </div>
        ...
        <div className="flex justify-end gap-4 pt-6">
          <Button
            ...
            asChild
          >
            <Link href="/credit-notes">Отказ</Link>
          </Button>
          <Button
            type="submit"
            disabled={isLoading}
