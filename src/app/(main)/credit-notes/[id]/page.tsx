      ...
      <div className="grid gap-6 md:grid-cols-3">
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(creditNote.items || []).map((item: CreditNoteItem) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-red-500/5 border-red-500/20">
            <CardHeader>
              <CardTitle className="text-lg text-red-600">Сума</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Сума без ДДС</span>
                <span>{formatCurrency(Number(creditNote.subtotal))}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">ДДС</span>
                <span>{formatCurrency(Number(creditNote.taxAmount))}</span>
              </div>
              <Separator />
              <div className="flex justify-between font-bold text-lg">
                <span>Общо</span>
                <span className="text-red-600">-{formatCurrency(Number(creditNote.total))}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span>{format(new Date(creditNote.issueDate), 'd MMM yyyy', { locale: bg })}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Валута</span>
                <span>{creditNote.currency || 'EUR'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Създадено</span>
                <span>{format(new Date(creditNote.createdAt), 'd MMM yyyy, HH:mm', { locale: bg })}</span>
              </div>
            </CardContent>
          </Card>
        </div>
