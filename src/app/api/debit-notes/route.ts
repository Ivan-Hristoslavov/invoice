...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate debit note number using Bulgarian format (per-user, like invoices)
    const currentYear = new Date().getFullYear();
    const startOfYear = new Date(currentYear, 0, 1).toISOString();
    
    // Count debit notes for this user in the current year
    ...
    const sequence = (debitNoteCount || 0) + 1;
    const companyEik = company.bulstatNumber || undefined;
    const debitNoteNumber = generateBulgarianInvoiceNumber(sequence, companyEik, 'debit-note');
