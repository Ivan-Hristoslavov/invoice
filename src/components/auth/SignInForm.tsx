...
import { EyeIcon, EyeOffIcon, LockIcon, MailIcon, ArrowRight, MailPlus } from "lucide-react";
...
export function SignInForm() {
  ...
  const credentialLock = useAsyncLock();
  const googleLock = useAsyncLock();
  const resendLock = useAsyncLock();
  ...
  return (
    <div className="w-full animate-in fade-in duration-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isEmailNotVerified && (
              <div className="space-y-2">
                <Button
                  ...
                  disabled={resendLock.isPending}
                  loading={resendLock.isPending}
                >
                  {!resendLock.isPending ? <MailPlus className="mr-2 h-3.5 w-3.5" /> : null}
        ...
        <Button
          type="submit"
          className="h-12 w-full rounded-2xl border-0 text-sm font-semibold text-white shadow-lg shadow-emerald-500/20 gradient-primary hover:shadow-md hover:ring-2 hover:ring-emerald-400/25"
          disabled={credentialLock.isPending}
          loading={credentialLock.isPending}
        >
          {!credentialLock.isPending ? <ArrowRight className="ml-2 h-4 w-4" /> : null}
          Влез в акаунта
        </Button>
        ...
        <Button
          ...
          disabled={googleLock.isPending}
          loading={googleLock.isPending}
        >
          {!googleLock.isPending ? (
