...
// Генерира PDF за фактура и връща Buffer
export async function exportInvoicePdfBuffer(invoiceId: string): Promise<{ buffer: Buffer, filename: string }> {
  ...
  // Зареждане на шрифтовете
  const regularFontResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/fonts/Roboto-Regular.ttf`);
  const regularFontData = await regularFontResponse.arrayBuffer();
  const boldFontResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/fonts/Roboto-Bold.ttf`);
  const boldFontData = await boldFontResponse.arrayBuffer();
  ...
  // Връщаме Buffer вместо да сваляме файла
  const pdfBuffer = Buffer.from(doc.output('arraybuffer'));
  return {
    buffer: pdfBuffer,
    filename: `Фактура-${result.invoiceNumber}.pdf`
  };
}

// Генерира PDF за фактура от обект и връща Buffer
export async function generateInvoicePdfBuffer(invoice: any): Promise<{ buffer: Buffer, filename: string }> {
  const { default: jsPDF } = await import('jspdf');
  ...
  doc.setFontSize(10);
  doc.setFont('Roboto', 'normal');
  doc.text(`Дата на издаване: ${new Date(invoice.issueDate || new Date()).toLocaleDateString('bg-BG')}`, margin, yPos);
  doc.text(`Място на издаване: ${invoice.placeOfIssue || 'София'}`, pageWidth - margin - 60, yPos);
  yPos += 7;
  doc.text(`Дата на дан. събитие: ${new Date(invoice.supplyDate || invoice.issueDate || new Date()).toLocaleDateString('bg-BG')}`, margin, yPos);
  ...
  if (invoice.company?.bankAccount) {
    yPos += 7;
    doc.text(`Банка: ${invoice.company.bankAccount.bankName || ''}`, margin, yPos);
    yPos += 7;
    doc.text(`IBAN: ${invoice.company.bankAccount.iban || ''}`, margin, yPos);
    doc.text(`BIC: ${invoice.company.bankAccount.bic || ''}`, margin, yPos);
  }
  yPos += 15;
  doc.setFontSize(9);
  doc.setFont('Roboto', 'bold');
  doc.text('Основание за издаване:', margin, yPos);
  doc.setFont('Roboto', 'normal');
  doc.text(invoice.basis || 'Договор за доставка на стоки/услуги', margin + 45, yPos);
  yPos += 7;
  doc.setFont('Roboto', 'bold');
  doc.text('Начин на плащане:', margin, yPos);
  doc.setFont('Roboto', 'normal');
  doc.text(invoice.paymentMethod || 'Банков превод', margin + 45, yPos);
  yPos += 7;
  doc.setFont('Roboto', 'bold');
  doc.text('Срок на плащане:', margin, yPos);
  doc.setFont('Roboto', 'normal');
  const dueDate = invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString('bg-BG') : 'При получаване';
  doc.text(dueDate, margin + 45, yPos);
  if (invoice.deliveryTerms) {
    ...
    doc.text('Условия на доставка:', margin, yPos);
    doc.setFont('Roboto', 'normal');
    doc.text(invoice.deliveryTerms, margin + 45, yPos);
  }
  if (invoice.discount > 0) {
    ...
    doc.text('Основание за отстъпка:', margin, yPos);
    doc.setFont('Roboto', 'normal');
    doc.text(invoice.discountReason || 'Търговска отстъпка', margin + 45, yPos);
  }
  if (invoice.company?.bankAccount) {
    yPos += 15;
    doc.setFillColor(240, 240, 240);
    ...
    doc.text('Банкова информация:', margin + 2, yPos + 7);
    doc.setFont('Roboto', 'normal');
    const bank = invoice.company.bankAccount;
    doc.text(`Банка: ${bank.bankName || ''}`, margin + 2, yPos + 15);
    doc.text(`IBAN: ${bank.iban || ''}`, margin + contentWidth/2, yPos + 15);
    doc.text(`BIC: ${bank.bic || ''}`, margin + 2, yPos + 23);
  }
  ...
  return {
    buffer: pdfBuffer,
    filename: `Фактура-${invoice.invoiceNumber}.pdf`
  };
}
