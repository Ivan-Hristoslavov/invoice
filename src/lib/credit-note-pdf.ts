...
const COLORS = {
  primary: { r: 239, g: 68, b: 68 }, // Red-500 for credit notes
  primaryDark: { r: 220, g: 38, b: 38 }, // Red-600
...
export async function generateCreditNotePdfServer(creditNote: any): Promise<Buffer> {
  ...
  // Credit Note badge (Top Center)
  doc.setFont('Roboto', 'bold');
  doc.setFontSize(14);
  doc.setTextColor(COLORS.primary.r, COLORS.primary.g, COLORS.primary.b);
  
  const badgeText = 'КРЕДИТНО ИЗВЕСТИЕ';
  ...
  doc.setFontSize(24);
  doc.setTextColor(COLORS.primary.r, COLORS.primary.g, COLORS.primary.b);
  doc.text('КРЕДИТНО', pageWidth - margin, yPos, { align: 'right' });
  ...
  doc.text(`№ ${creditNote.creditNoteNumber || '---'}`, pageWidth - margin, yPos, { align: 'right' });
  ...
  if (creditNote.company) {
    ...
    doc.setFontSize(14);
    doc.setTextColor(COLORS.dark.r, COLORS.dark.g, COLORS.dark.b);
    doc.text(creditNote.company.name || '', margin, yPos);
    ...
    if (creditNote.company.address) {
      yPos += 6;
      doc.text(creditNote.company.address, margin, yPos);
    }
    if (creditNote.company.city) {
      yPos += 5;
      doc.text(creditNote.company.city, margin, yPos);
    }
  }
  ...
  const issueDate = creditNote.issueDate ? new Date(creditNote.issueDate).toLocaleDateString('bg-BG') : new Date().toLocaleDateString('bg-BG');
  const invoiceRef = creditNote.invoice?.invoiceNumber || 'N/A';
  const invoiceDate = creditNote.invoice?.issueDate ? new Date(creditNote.invoice.issueDate).toLocaleDateString('bg-BG') : '';
  ...
  if (creditNote.company) {
    doc.setFont('Roboto', 'bold');
    doc.text(creditNote.company.name || '', margin + 5, supplierY);
    supplierY += 5;
    ...
    if (creditNote.company.bulstatNumber) {
      doc.text(`ЕИК: ${creditNote.company.bulstatNumber}`, margin + 5, supplierY);
      supplierY += 4;
    }
    if (creditNote.company.vatRegistrationNumber) {
  ...
  if (creditNote.client) {
    doc.setFont('Roboto', 'bold');
    doc.text(creditNote.client.name || '', clientBoxX + 5, clientY);
    clientY += 5;
    ...
    if (creditNote.client.bulstatNumber) {
    ...
    if (creditNote.client.address) {
      doc.text(creditNote.client.address, clientBoxX + 5, clientY);
      clientY += 4;
    }
  ...
  if (creditNote.reason) {
    ...
    doc.setFont('Roboto', 'normal');
    doc.setTextColor(COLORS.dark.r, COLORS.dark.g, COLORS.dark.b);
    doc.text(creditNote.reason, margin + 5, yPos + 11);
  ...
  // Totals box with red border for credit notes
  doc.setDrawColor(COLORS.primary.r, COLORS.primary.g, COLORS.primary.b);
  doc.setLineWidth(0.5);
  ...
  doc.setTextColor(COLORS.dark.r, COLORS.dark.g, COLORS.dark.b);
  doc.text(formatCurrency(Number(creditNote.subtotal), currency), totalsX + totalsWidth - 5, totalsY + 8, { align: 'right' });
  
  // Tax
  doc.setTextColor(COLORS.muted.r, COLORS.muted.g, COLORS.muted.b);
  doc.text('ДДС:', totalsX + 5, totalsY + 16);
  doc.setTextColor(COLORS.dark.r, COLORS.dark.g, COLORS.dark.b);
  doc.text(formatCurrency(Number(creditNote.taxAmount), currency), totalsX + totalsWidth - 5, totalsY + 16, { align: 'right' });
  ...
  doc.text(`-${formatCurrency(Number(creditNote.total), currency)}`, totalsX + totalsWidth - 5, totalsY + 30, { align: 'right' });
  ...
  doc.setFontSize(8);
  doc.setTextColor(COLORS.muted.r, COLORS.muted.g, COLORS.muted.b);
  doc.text(`Кредитно известие ${creditNote.creditNoteNumber} към фактура ${creditNote.invoice?.invoiceNumber || 'N/A'}`, margin, footerY);
